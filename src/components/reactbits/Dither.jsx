...
function DitheredWaves({
  ...
  useEffect(()=>{const move=e=>{if(!enableMouseInteraction)return;const rect=gl.domElement.getBoundingClientRect();if(!rect.width||!rect.height)return;const dpr=gl.getPixelRatio();mouseRef.current.set((e.clientX-rect.left)/rect.width*size.width*dpr,(e.clientY-rect.top)/rect.height*size.height*dpr)};window.addEventListener('pointermove',move);return()=>window.removeEventListener('pointermove',move)},[gl,size,enableMouseInteraction]);
  ...
  const handlePointerMove = e => {
    ...
    const dpr = gl.getPixelRatio();
    mouseRef.current.set(((e.clientX - rect.left) / rect.width) * size.width * dpr, ((e.clientY - rect.top) / rect.height) * size.height * dpr);
  };
